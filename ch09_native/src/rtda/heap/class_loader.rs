...
impl ClassLoader {
    ...
    fn load_basic_class(&mut self, _self: &Rc<RefCell<Self>>) {
    ...
    fn load_primitive_classes(&mut self, _self: &Rc<RefCell<Self>>) {
        for (primitive_class, _) in PRIMITIVE_TYPES.iter() {
            self.load_primitive_class(_self, (*primitive_class).into());
        }
    }

    fn load_primitive_class(&mut self, _self: &Rc<RefCell<Self>>, class_name: String) {
